...
public:
    constraintNode(std::shared_ptr<binaryExpressionNode> constraint) :
        constraints{std::vector<std::shared_ptr<binaryExpressionNode>>{std::move(constraint)}} {}
    ...
    bool isSatisfiable(const variableNode *var) {
        ...
        z3::expr name = (var->getType() == intType) ? c.int_const(n) : c.bool_const(n);
        ...
        for (auto &e : constraints) {
            ...
            switch (e->getOperator()) {
                ...
                case GEQ:
                    s.add(name >= std::stoi(val));
                case EQ:
                    s.add(name == ((var->getType() == intType) ? std::stoi(val) : (val == "true")));
                case NEQ:
                    s.add(name == ((var->getType() == intType) ? std::stoi(val) : (val == "true")));
                default:
