...
public:
    writeNode(std::shared_ptr<expressionNode> v, std::shared_ptr<expressionNode> e) : _e{std::move(e)}, _v{std::move(v)} {
        type = (_v->getType() == intType && _e->getType() == intType) ? okType : errorType;
        setNodeType(Write);
    }
    writeNode(Type t, std::shared_ptr<expressionNode> v, std::shared_ptr<expressionNode> e) : _e{std::move(e)}, _v{std::move(v)} {
        setType(t);
        setNodeType(Write);
    }

    expressionNode *getVar() const {return _v.get();};
    expressionNode *getExpr() const {return _e.get();};

private:
    std::shared_ptr <expressionNode> _v;
