...
public:
    piNode(Type t, std::string variable, int _num, const std::vector<std::pair<std::string, std::string>>& possible_variables)
        ...
        for (const auto &p : possible_variables) {
            _possible_variables.emplace_back(option(p.first, p.second, "-option-" + std::to_string(++i) + '-' + name));
        }
        setType(t);
        set_linenum(-1);
        setNodeType(Pi);
    ...
    piNode(phiNode *phi) : _variable{phi->getOriginalName()}, name{phi->getName()} {
        ...
        num = std::stoi(sub);
        int i = 0;
        for (const auto &v : *phi->get_variables()) _possible_variables.emplace_back(v.var, v.var_boolname, "-option-" + std::to_string(++i) + '-' + name);
        setType(phi->getType());
        set_linenum(-1);
        setNodeType(Pi);
        setSSA(true);
    ...
    void addVariable(std::pair<std::string, std::string> var) {_possible_variables.emplace_back(var.first, var.second, "-option-" +  std::to_string(_possible_variables.size()+1) + '-' + name);}
